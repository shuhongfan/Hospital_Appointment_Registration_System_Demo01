...
//        判断是否取消
        ...
        if (quitTime.isBeforeNow()) { //当前时间大约退号时间，不能取消预约
...
//        根据医院接口返回数据
        if(result.getInteger("code") != 200) { // 失败
            throw new YyghException(result.getString("message"), ResultCodeEnum.FAIL.getCode());
        } else {
        //是否支付 退款
//            判断当前顶点是否可以取消
